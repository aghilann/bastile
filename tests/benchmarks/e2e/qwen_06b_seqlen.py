"""
Qwen3-0.6B Sequence Length Sweep: PyTorch vs Liger vs Bastile

Real pretrained Qwen/Qwen3-0.6B benchmarked across doubling sequence lengths.
...
Usage:
  ...
  python -u -m tests.benchmarks.e2e.qwen_06b_seqlen

  # Custom GPU assignment:
  python -u -m tests.benchmarks.e2e.qwen_06b_seqlen --gpus 2,3,4

  # Single-GPU sequential (if only 1 GPU available):
  python -u -m tests.benchmarks.e2e.qwen_06b_seqlen --sequential

  # Internal: run one phase (called by subprocess):
  python -u -m tests.benchmarks.e2e.qwen_06b_seqlen --phase pytorch --gpu 0
"""
...
def run_phase(phase: str, output_file: str):
    ...
    for seq_len in SEQ_LENS:
        ...
        r = run_benchmark(name, setup_fn=setup_fn, seq_len=seq_len)
...
def run_parallel(gpus: list[int]):
    ...
    tmp_dir = tempfile.mkdtemp(prefix="bench_qwen06b_")
    ...
    for phase, gpu in zip(phases, gpus):
        ...
        cmd = [
            ...
            "-u",
            "-m",
            "tests.benchmarks.e2e.qwen_06b_seqlen",
            "--phase",
            phase,
            "--gpu",
...
def plot_results(
    ...
    _bar_chart(
        title="Qwen3-0.6B E2E Training — Throughput (tokens/sec)",
        ylabel="Tokens / sec",
        filename="bench_06b_throughput.png",
    ...
    _bar_chart(
        title="Qwen3-0.6B E2E Training — Latency (ms / iteration)",
        ylabel="ms / iteration",
        filename="bench_06b_latency.png",
    ...
    _bar_chart(
        title="Qwen3-0.6B E2E Training — Peak GPU Memory (GB)",
        ylabel="Peak Memory (GB)",
        filename="bench_06b_memory.png",
...
def main():
    ...
    # Full benchmark
    print("=" * 110)
    print("  Qwen3-0.6B Sequence Length Sweep: PyTorch vs Liger vs Bastile")
    print("=" * 110)
    print(f"\n  Model: {MODEL_ID}, batch_size={BATCH_SIZE}")
    print(f"  Warmup: {WARMUP_ITERS} iters, Duration: {DURATION_SEC}s per run")
    print(f"  Sequence lengths: {SEQ_LENS}")
    print("  Pretrained weights loaded from Hugging Face cache")
